...
export default function MapPage() {
  ...
  const [selectedStalls, setSelectedStalls] = useState([]); // Changed to array for multi-select
  const [selectedStall, setSelectedStall] = useState(null); // For single reservation confirmation
  ...
  }, [selectedStalls, selectedStall, hoveredStallId, stalls]);
  ...
  const loadStalls = async () => {
    try {
      ...
      // Add token if available (though endpoint should work without it)
      if (token) {
      ...
      if (!res.ok) {
        ...
        console.error('Failed to load stalls: HTTP', res.status, errorText);
        setMessage(`Failed to load stalls (HTTP ${res.status}). The backend may need to be restarted.`);
        return;
      }
      
      const data = await res.json();
      console.log('Loaded stalls:', data.length, 'stalls');
      setStalls(data);
      setMessage(''); // Clear any previous error messages
    } catch (err) {
      console.error('Failed to load stalls:', err);
      setMessage('Failed to load stalls. Please check if the backend is running on port 8081.');
    }
  };

  const initializePolygonMapping = () => {
    if (!svgRef.current || stalls.length === 0) {
      console.log('Cannot initialize: svgRef=', !!svgRef.current, 'stalls.length=', stalls.length);
      return;
    }
    
    const polygons = svgRef.current.querySelectorAll('polygon');
    console.log('Found', polygons.length, 'polygons, mapping', Math.min(stalls.length, 26), 'stalls');
    const map = new Map();
    
    // First, set default styles for ALL polygons (make them invisible and non-interactive)
    polygons.forEach((polygon) => {
      ...
      // Set default transparent style for all polygons initially
    ...
    // Map first 26 stalls (A-Z) to first 26 polygons
    stalls.slice(0, 26).forEach((stall, index) => {
      if (index < polygons.length) {
        const polygon = polygons[index];
        map.set(polygon, stall);
        
        // Enable pointer events for mapped polygons
        ...
        // Store handlers on the polygon for cleanup
    ...
    polygonStallMapRef.current = map;
  };
  ...
  const updatePolygonStyles = () => {
    ...
    polygons.forEach((polygon) => {
      ...
      if (stall) {
        // Base styling
        polygon.style.fill = stall.reserved 
          ? 'rgba(100, 100, 100, 0.7)' 
          : getStallFillColor(stall);
        ...
        const isSelected = selectedStalls.some(s => s.id === stall.id) || 
                          (selectedStall && selectedStall.id === stall.id);
        
        // Check if hovered (but not if selected, selected takes priority)
        ...
        if (isSelected) {
          ...
          polygon.style.opacity = '1';
        } else if (isHovered && !stall.reserved) {
          // Hovered state - green border
          ...
          polygon.style.filter = 'none';
          polygon.style.opacity = stall.reserved ? '0.6' : '1';
        }
      } else {
        // Unmapped polygons - make them completely invisible and non-interactive
        polygon.style.fill = 'transparent';
        polygon.style.stroke = 'none';
        polygon.style.cursor = 'default';
        polygon.style.pointerEvents = 'none';
  ...
  const getStallFillColor = (stall) => {
    ...
    switch (stall.size) {
      case 'SMALL': return 'rgba(255, 235, 59, 0.6)'; // Yellow
      case 'MEDIUM': return 'rgba(255, 152, 0, 0.6)'; // Orange
      case 'LARGE': return 'rgba(76, 175, 80, 0.6)'; // Green
  ...
  const handlePolygonClick = (stall) => {
    ...
    const isAlreadySelected = selectedStalls.some(s => s.id === stall.id);
    
    if (isAlreadySelected) {
      // Deselect
      setSelectedStalls(selectedStalls.filter(s => s.id !== stall.id));
    } else {
      // Check if user can select more (max 3 total reservations)
  ...
  const handleReserveSelected = () => {
    ...
    // Reserve first stall and show confirmation
    if (selectedStalls.length > 0) {
      setSelectedStall(selectedStalls[0]);
      setShowConfirm(true);
    }
  };
  ...
  const handleReserveAll = async () => {
    ...
    try {
      ...
      if (successCount > 0) {
        setMessage(`Successfully reserved ${successCount} stall(s)! Check your email for QR codes.`);
        setSelectedStalls([]);
        setTimeout(() => {
          loadStalls();
          loadUserReservations();
  ...
  const handleConfirmReservation = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage('Reservation confirmed! Check your email for the QR code.');
        setShowConfirm(false);
        setSelectedStall(null);
        // Remove from selected stalls if it was there
        setSelectedStalls(selectedStalls.filter(s => s.id !== selectedStall.id));
        setTimeout(() => {
          loadStalls();
          loadUserReservations();
          router.push('/home');
        ...
        setMessage(data.error || 'Failed to reserve stall');
      }
    } catch (err) {
      setMessage('Failed to connect to server');
    } finally {
  ...
  const canSelectMore = selectedStalls.length < remainingSlots;

  return (
    <div style={styles.container}>
      <h1>Stall Reservation Map</h1>
      <p>
        Click on stall areas to select them. You can reserve up to 3 stalls total.
        {userReservations > 0 && <span> You have {userReservations} existing reservation(s).</span>}
        {remainingSlots > 0 && <span> You can select up to {remainingSlots} more.</span>}
      </p>
      
      {/* Selection Panel */}
      {selectedStalls.length > 0 && (
        <div style={styles.selectionPanel}>
          <h3>Selected Stalls ({selectedStalls.length}/{remainingSlots})</h3>
          <div style={styles.selectedStallsList}>
            {selectedStalls.map(stall => (
              <div key={stall.id} style={styles.selectedStallItem}>
                <span><strong>{stall.name}</strong> - {stall.size}</span>
                <button
          ...
          <div style={styles.selectionActions}>
            ...
            >
              {loading ? 'Processing...' : `Reserve All (${selectedStalls.length})`}
            </button>
            <button
      ...
      <div style={styles.legend}>
        <div style={styles.legendItem}>
          <div style={{...styles.legendBox, backgroundColor: 'rgba(255, 235, 59, 0.6)'}}></div>
          <span>Small</span>
        </div>
        <div style={styles.legendItem}>
          <div style={{...styles.legendBox, backgroundColor: 'rgba(255, 152, 0, 0.6)'}}></div>
          <span>Medium</span>
        </div>
        <div style={styles.legendItem}>
          <div style={{...styles.legendBox, backgroundColor: 'rgba(76, 175, 80, 0.6)'}}></div>
          <span>Large</span>
        </div>
        <div style={styles.legendItem}>
          <div style={{...styles.legendBox, backgroundColor: 'rgba(100, 100, 100, 0.7)'}}></div>
          <span>Reserved</span>
        </div>
      </div>
      ...
      {hoveredStall && (
        <div style={styles.tooltip}>
          <strong>{hoveredStall.name}</strong> - {hoveredStall.size}
          {hoveredStall.reserved && <span style={{color: '#f44336'}}> (Reserved)</span>}
        </div>
      )}
      ...
      <div style={styles.mapWrapper}>
        ...
        >
          ...
          {/* All 436 polygons from map.html */}
      ...
      {message && (
        <div style={message.includes('confirmed') ? styles.successMessage : styles.errorMessage}>
          {message}
        </div>
      )}

      {/* Confirmation Popup */}
      {showConfirm && selectedStall && (
        <div style={styles.overlay} onClick={() => { setShowConfirm(false); setSelectedStall(null); }}>
          <div style={styles.popup} onClick={(e) => e.stopPropagation()}>
            <h2>Confirm Reservation</h2>
            <p>Do you want to reserve stall <strong>{selectedStall.name}</strong> ({selectedStall.size})?</p>
